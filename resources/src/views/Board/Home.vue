...
import BikerDataService from "../../services/BikerDataService";
...
export default {
    ...
    async mounted(id = "616f4b59f16b5e46cb68c561") {
        console.log("mounted");
        const res_0 = await BikerDataService.getAvailableParcels(id);
        this.columns[0].items = res_0.data;

        const res_1 = await BikerDataService.getPickedUpParcels(id);
        this.columns[1].items = res_1.data;

        const res_2 = await BikerDataService.getDeliveredParcels(id);
